...
async function main(): Promise<void> {
	...
	await downloadPackage(coreApi, feedId, packageId, downloadPath, version);
}
...
export async function downloadPackage(coreApi, feedId: string, packageId: string, downloadPath: string, version) {
	...
	await new Promise((resolve, reject) => {
		coreApi.restClient.get(packageUrl, ApiVersion, null, { responseIsCollection: false }, async function (error, status, result) {
			if (!!error && status != 200) {
				reject(tl.loc("FailedToGetPackageUrl", error));
			}
			...
			if (packageType == "nuget") {
				...
				console.log(tl.loc("StartingDownloadOfPackage", packageName));
				await downloadNugetPackage(downloadUrl, coreApi, zipLocation);
				console.log(tl.loc("ExtractingNugetPackage", packageName));
...
export async function getPackageUrl(vsoClient: vsom.VsoClient, feedId, packageId): Promise<string> {
...
export async function getDownloadUrl(vsoClient: vsom.VsoClient, feedId, packageName, version): Promise<string> {
	var NugetArea = "NuGet"
...
export async function downloadNugetPackage(downloadUrl: string, coreApi, downloadPath: string): Promise<void> {
...
export async function unzip(zipLocation, unzipLocation): Promise<void> {
